    ...
    <el-dialog :title="preview.title" :visible.sync="preview.open" width="80%" top="5vh" append-to-body class="scrollbar">
      <el-tabs v-model="preview.activeName">
        <el-tab-pane v-for="(value, key) in preview.data"
          :label="key.substring(key.lastIndexOf('/') + 1, key.indexOf('.scriban'))"
          :name="key.substring(key.lastIndexOf('/') + 1, key.indexOf('.scriban'))" :key="key">
  ...
  methods: {
    ...
    handlePreview(row) {
      previewTable(row.tableId).then(response => {
        ...
        this.preview.activeName = "api.js";
      });
    },
    /** 高亮显示 */
    highlightedCode(code, key) {
      const vmName = key.substring(key.lastIndexOf("/") + 1, key.indexOf(".scriban"));
